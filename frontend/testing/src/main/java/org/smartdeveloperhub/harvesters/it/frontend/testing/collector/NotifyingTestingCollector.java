...
public final class NotifyingTestingCollector implements TestingCollector {
	...
	private final class InnerOwner implements ActivityContext {
		...
		public String resolve(final String path, final Object... args) {
			return NotifyingTestingCollector.this.target.resolve(String.format(path,args)).toString();
		}

		@Override
		public void onActivity(final Activity<?> activity) {
			final ActivityListener current=NotifyingTestingCollector.this.listener;
			if(current!=null) {
	...
	public NotifyingTestingCollector(final CollectorConfiguration config) {
		this.config = config;
	...
	@Override
	public NotifyingTestingCollector registerListener(final ActivityListener listener) {
